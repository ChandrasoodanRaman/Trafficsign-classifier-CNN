...
def classify(file_path):
    ...
    if len(image_array.shape) == 2:
        image_array = numpy.stack((image_array,) * 3, axis=-1)
    # Normalize the image pixel values to be in the range [0, 1]
    image_array = image_array / 255.0
    # Expand the dimensions to match the expected input shape (None, 30, 30, 3)
    ...
    pred_class_index = numpy.argmax(pred_probabilities)
